...
for n = 1:trials     %This loop will generate the trials for each condition
    ...
    correct_m(n,2) = toc;        %Reaction time is stored in the correct matrix
    if answer == 's'
        answer = 1;
    elseif answer == 'l'
        answer = 0;
    ...
    raw_m(n,1) = answer;         %an incorrect answer, but providing additional
    if raw_m(n,1) == raw_m(n,2)  %information
        correct_m(n,1) = 1;
    end
    delete(gca)
end
